...
def index():
    ...
    try:
        ...
        transactions = db_s.query(Transaction).filter(
        ...
        expenses_by_category = db_s.query(
            Category.category_name,
            Category.color,
            func.sum(Transaction.amount).label('total')
        ).join(Transaction).filter(
        ...
        for i in range(11, -1, -1):
            ...
            month_income = sum(t.amount for t in db_s.query(Transaction).filter(
                Transaction.user_id == user.id,
                Transaction.transaction_type == 'income',
            ...
            month_expenses = sum(t.amount for t in db_s.query(Transaction).filter(
                Transaction.user_id == user.id,
                Transaction.transaction_type == 'expense',
                Transaction.transaction_date >= first_of_month,
                Transaction.transaction_date <= last_of_month
    ...
    try:
        ...
        try:
            ...
            if export_type in ['income', 'expense']:
                query = query.filter(Transaction.transaction_type == export_type)
